...
def transform():
    ...
    result = df.groupby(['Product','Year','Month','Retailer','SalesMethod'])[['UnitsSold','Revenue','OperatingProfit']].sum().reset_index()
    
    result.to_csv('/tmp/transformed_data_adidas.csv', index=False)

def load():
    df = pd.read_csv('/tmp/transformed_data_adidas.csv')
    csv_path = os.path.join(dag_path, 'OLAP_data_factsales.csv')
    
    df.to_csv(csv_path, index=False)

with DAG(
    dag_id='etl_fact_sales_kel_4',
