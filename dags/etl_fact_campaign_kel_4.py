...
def transform():
    ...
    result = df.groupby(['Product','Year','Month'])[['UnitsSold','Revenue','OperatingProfit']].sum().reset_index()
    result['Roi'] = (result['OperatingProfit'] / result['Revenue']) * 100
    result['Roi'] = result['Roi'].round(1).astype(str) + '%'

    result.to_csv('/tmp/transformed_data_adidas.csv', index=False)

def load():
    df = pd.read_csv('/tmp/transformed_data_adidas.csv')
    csv_path = os.path.join(dag_path, 'OLAP_data_factcampaign.csv')
    
    df.to_csv(csv_path, index=False)

with DAG(
    dag_id='etl_fact_campaign_kel_4',
